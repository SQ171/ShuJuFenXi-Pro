...
from visualization.common import base_layout, DIMENSION_COLORS
...
def plot_force_eff_timeseries(df: pd.DataFrame, highlight_anomalies: bool = True) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", line=dict(color="#DC143C", width=1.5),
        name="系统力效",
    ))
    if highlight_anomalies and "is_outlier" in df.columns:
        ...
        if len(outliers) > 0:
            fig.add_trace(go.Scatter(
                x=outliers[x_col], y=outliers[COL_SYS_EFF],
...
def plot_multi_signal_timeseries(df: pd.DataFrame, signals: list[str],
    ...
    if COL_RUNTIME_HOURS not in sub.columns and COL_CUMULATIVE_RUNTIME in sub.columns:
        sub = sub.copy()
        sub[COL_RUNTIME_HOURS] = sub[COL_CUMULATIVE_RUNTIME] / 3600.0
